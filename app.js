...
/**
 * Main entry point.
 * Loads the moose modules, the moose api and run the express server as Domain master.
...
/**
 * The config object to handle moose modules and moose api.
  ...
  async.auto({
      ...
      Object.keys(config.modules).forEach(function (name) {
        tasks[name] = function (cb) {
          ...
          d.run(function () {
            ...
            var Class = require(config.modules[name]);
            var obj = new Class(cb, scope);
            modules.push(obj);
          });
        };
      });
    ...
    listen: ['ready', function (scope, cb) {
      scope.network.server.listen(scope.config.port, scope.config.address, function (err) {
        scope.logger.info('Moose node started: ' + scope.config.address + ':' + scope.config.port);

        if (!err) {
          if (scope.config.ssl.enabled) {
            scope.network.https.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, function (err) {
              scope.logger.info('Moose node https started: ' + scope.config.ssl.options.address + ':' + scope.config.ssl.options.port);
