...
node.normalizer = 100000000; // Use this to convert MOOSE amount to normal value
...
node.fees = {
	...
	songAddFee: node.constants.fees.song
};

// Existing delegate account
node.eAccount = {
	address: '10881167371402274308M',
...
// Random MOOSE amount
node.MOOSE = Math.floor(Math.random() * (100000 * 100000000)) + 1;
...
// Returns random MOOSE amount
node.randomMOOSE = function () {
...
node.randomAccount = function () {
	...
	account.publicKey = node.moose.crypto.getKeys(account.password).publicKey;
	account.address = node.moose.crypto.getAddress(account.publicKey);
