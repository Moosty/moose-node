...
describe('POST /peer/transactions', function () {
	...
	it('using valid headers should be ok', function (done) {
		...
		var transaction = node.moose.transaction.createTransaction(account.address, 1, node.gAccount.password);
	...
	it('using varying recipientId casing should go to same address', function (done) {
		var account = node.randomAccount();
		var keys = node.moose.crypto.getKeys(account.password);
		var address = node.moose.crypto.getAddress(keys.publicKey);

		var transaction = node.moose.transaction.createTransaction(address, 100000000, node.gAccount.password);
		postTransaction(transaction, function (err, res) {
			node.expect(res.body).to.have.property('success').to.be.ok;

			node.onNewBlock(function (err) {
				var transaction2 = node.moose.transaction.createTransaction(address.toLowerCase(), 100000000, node.gAccount.password);
	...
	it('using transaction with undefined recipientId should fail', function (done) {
		var transaction = node.moose.transaction.createTransaction(undefined, 1, node.gAccount.password);
	...
	it('using transaction with invalid recipientId should fail', function (done) {
		var transaction = node.moose.transaction.createTransaction('0123456789001234567890M', 1, node.gAccount.password);
	...
	it('using transaction with negative amount should fail', function (done) {
		var transaction = node.moose.transaction.createTransaction('1M', -1, node.gAccount.password);
	...
	it('using invalid passphrase should fail', function (done) {
		var transaction = node.moose.transaction.createTransaction('12M', 1, node.gAccount.password);
		transaction.recipientId = '1M';
		transaction.id = node.moose.crypto.getId(transaction);
	...
	it('when sender has no funds should fail', function (done) {
		var transaction = node.moose.transaction.createTransaction('1M', 1, 'randomstring');

		postTransaction(transaction, function (err, res) {
			node.expect(res.body).to.have.property('success').to.be.not.ok;
			node.expect(res.body).to.have.property('message').to.match(/Account does not have enough MOOSE: [0-9]+M balance: 0/);
			done();
		});
	});
	...
	it('when sender does not have enough funds should always fail', function (done) {
		...
		postTransaction(transaction, function (err, res) {
			...
			node.onNewBlock(function () {
				var count = 1;
				var transaction2 = node.moose.transaction.createTransaction(node.gAccount.address, 2, account.password);

				node.async.doUntil(function (next) {
					postTransaction(transaction2, function (err, res) {
						node.expect(res.body).to.have.property('success').to.be.not.ok;
						node.expect(res.body).to.have.property('message').to.match(/Account does not have enough MOOSE: [0-9]+M balance: 1e-8/);
						count++;
						return next();
					});
				}, function () {
	...
	it('using very large amount and genesis block id should fail', function (done) {
		var transaction = node.moose.transaction.createTransaction('12M', 10000000000000000, node.gAccount.password);
	...
	it('using overflown amount should fail', function (done) {
		var transaction = node.moose.transaction.createTransaction('12M', 184819291270000000012910218291201281920128129,
	...
	it('using float amount should fail', function (done) {
		var transaction = node.moose.transaction.createTransaction('12M', 1.3, node.gAccount.password);
