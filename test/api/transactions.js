...
function sendMOOSE (account, amount, done) {
...
before(function (done) {
	setTimeout(function () {
		sendMOOSE(account, node.randomMOOSE(), done);
	}, 2000);
});

before(function (done) {
	setTimeout(function () {
		sendMOOSE(account2, node.randomMOOSE(), done);
	}, 2000);
});
...
before(function (done) {
	setTimeout(function () {
		// Send 20 MOOSE
		sendMOOSE(account2, 20*100000000, done);
	}, 2000);
});
...
before(function (done) {
	setTimeout(function () {
		// Send 100 MOOSE
		sendMOOSE(account2, 100*100000000, done);
	}, 2000);
});
...
describe('GET /api/transactions', function () {
	...
	it('using minAmount with and:maxAmount ordered by amount and limited should be ok', function (done) {
		...
		var offset = 0;
		var orderBy = 'amount:asc';
		var minAmount = 20*100000000; // 20 MOOSE
		var maxAmount = 100*100000000; // 100 MOOSE
...
describe('PUT /api/transactions', function () {
	...
	it('using entire balance should fail', function (done) {
		openAccount({ secret: account.password }, function (err, res) {
			...
			putTransaction({
				...
				node.expect(res.body).to.have.property('success').to.be.not.ok;
				node.expect(res.body).to.have.property('error').to.match(/Account does not have enough MOOSE: [0-9]+M balance: [0-9.]+/);
				done();
			});
		});
	});
	...
	describe('to a cold address', function (done) {
		var recipientId = '13896491535841206186M';
