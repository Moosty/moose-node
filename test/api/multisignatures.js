...
function sendMOOSE (account, i, done) {
	var randomMOOSE = node.randomMOOSE();
	...
	node.put('/api/transactions/', {
		...
		if (res.body.success && i != null) {
			accounts[i].balance = randomMOOSE / node.normalizer;
		}
		done();
	});
}

function sendMOOSEFromMultisigAccount (amount, recipient, done) {
	node.put('/api/transactions/', {
...
before(function (done) {
	sendMOOSE(multisigAccount, null, done);
});
...
describe('PUT /api/multisignatures', function () {
	...
	it('using random passphase should fail', function (done) {
		...
		node.put('/api/multisignatures', validParams, function (err, res) {
			...
			node.expect(res.body).to.have.property('error').to.match(/Account does not have enough MOOSE: [0-9]+M balance: 0/);
			done();
		});
	});
...
describe('PUT /api/transactions', function () {
	...
	it('when group transaction is pending should be ok', function (done) {
		sendMOOSEFromMultisigAccount(100000000, node.gAccount.address, function (err, transactionId) {
			node.onNewBlock(function (err) {
